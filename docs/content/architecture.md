...
| Publishes only a symbol's **body** range, and language servers disagree about where a body starts — gopls anchors it at the `func` line, rust-analyzer at the doc comment above it | Scans the reported range for the line that actually writes the name, skipping comment lines first, instead of trusting its first line. Without it the two languages answered different questions from the same file: overview put `CANDIDATES` at line 48 and definition at 42, and neither said so |
