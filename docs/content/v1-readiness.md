...
This page is the acceptance record for the published v1.0.2 release. It separates what is
...
`go test -race -count=1 ./...`, policy anchors and the `1.0.2`/`3.1.0` build
...
passed on macOS arm64. This validates the public `1.0.2` release lifecycle;
...
the policy gate and the `1.0.2` release smoke passed. The timing margins in
