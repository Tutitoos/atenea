...
while IFS= read -r path; do
 case "$path" in
  CHANGELOG.md|README.md|pkg/contract/version.go|internal/metrics/migrations/0005_raw_failure.sql) ;;
  ...
  internal/config/config.go|internal/config/config_test.go|internal/config/retired_serena_test.go|cmd/atenea/headroom_wrap.go|cmd/atenea/main.go|cmd/atenea/retired_serena_test.go|tools/mcp-agree|scripts/provider-matrix-check.sh|scripts/serena-retirement-check.sh|scripts/v1-readiness.sh) ;;
