...
// TestAuditCostLostOnExit checks the regression scenario: audit cost lost on exit.
func TestAuditCostLostOnExit(t *testing.T) {
	...
	out, err := newRunner(t, bin, codexTestTimeout).Run(t.Context(), request(t, t.TempDir(), map[string]any{"query": "test"}))
