...
func (s *Store) Begin(ctx context.Context, e Event) (context.Context, *Call) {
	...
	if err == nil && time.Since(s.compacted) > time.Hour {
		err = s.compact(db, time.Now())
		if err == nil {
			s.compacted = time.Now()
		}
	}
	if err == nil {
		_, err = db.Exec(`INSERT INTO events(id,parent,level,tool,provider,repository,at) VALUES(?,?,?,?,?,?,?)`, e.ID, e.Parent, e.Level, e.Tool, e.Provider, e.Repository, e.At.UnixMicro())
	}
	if err != nil {
		s.dropped.Add(1)
	}
...
func (s *Store) compact(db *sql.DB, now time.Time) error {
 ...
 sum(CASE WHEN outcome!='cancel' THEN duration ELSE 0 END),sum(outcome!='cancel'),
 max(CASE WHEN outcome!='cancel' THEN duration ELSE 0 END),max(at)
